...
def get_user_by_id(user_id):
    conn = get_db()
    cur = conn.execute("SELECT id, username, password_hash, is_approved, email FROM users WHERE id = ?", (user_id,))
    row = cur.fetchone()
    return {"id": row[0], "username": row[1], "password_hash": row[2], "is_approved": row[3], "email": row[4]} if row else None
