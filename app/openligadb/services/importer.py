...
def import_matches():
    ...
    for match in data:
        ...
        existing = conn.execute('SELECT last_update FROM matches WHERE id = ?', (match['matchID'],)).fetchone()
        ...
        if existing:
            if existing[0] != match['lastUpdateDateTime']:
                ...
                ''', (
                    ...
                    match.get('numberOfViewers'), match['matchID']
                ))
        else:
            conn.execute('''
                INSERT INTO matches (
                ...
                (
                    match['matchID'], league['id'], group['id'],
        ...
        # Nur aktualisieren, wenn sich etwas verändert hat
        if not existing or existing[0] != match['lastUpdateDateTime']:
            conn.execute('DELETE FROM match_results WHERE match_id = ?', (match['matchID'],))
